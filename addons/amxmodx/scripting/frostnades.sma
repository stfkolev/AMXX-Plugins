...
new const MODEL_FROZEN[]	= "models/frostnades/iceblock.mdl";
...
new const SPRITE_TRAIL[2][]	= { "sprites/frostnade/frost_trail.spr", "sprites/frostnade/frost_trail2.spr" };
new const SPRITE_SMOKE[]	= "sprites/steam1.spr";
new const SPRITE_EXPLO[5][]	= { "sprites/frostnade/frost_explode.spr", "sprites/frostnade/frost_explode2.spr", "sprites/frostnade/frost_explode3.spr", "sprites/frostnade/frost_explode4.spr", "sprites/frostnade/frost_explode5.spr" };
new const SPRITE_GIB[3][]	= {"sprites/frostnade/frost_gib.spr", "sprites/frostnade/frost_gib2.spr", "sprites/frostnade/frost_gib3.spr" };
...
new maxPlayers, gmsgScreenFade, gmsgStatusIcon, gmsgBlinkAcct, gmsgAmmoPickup, gmsgTextMsg,
		gmsgWeapPickup, glassGibs, trailSpr, trailSpr2, smokeSpr, exploSpr, exploSpr2, exploSpr3, exploSpr4, exploSpr5, gibSpr, gibSpr2, gibSpr3, mp_friendlyfire, czero, bot_quota, czBotHams, fmFwdPPT,
...
public plugin_natives() {
	register_library("frostnades_fix");

	register_native("fn_is_user_chilled", "native_fn_is_user_chilled");
	register_native("fn_is_user_frozen", "native_fn_is_user_chilled");
	register_native("fn_get_chill_duration", "native_fn_get_chill_duration");
	register_native("fn_get_user_chillspeed", "native_fn_get_user_chillspeed");
}
...
public plugin_precache()
{
	...
	trailSpr 	= precache_model(SPRITE_TRAIL[0]);
	trailSpr2 	= precache_model(SPRITE_TRAIL[1]);
	smokeSpr 	= precache_model(SPRITE_SMOKE);
	exploSpr 	= precache_model(SPRITE_EXPLO[0]);
	exploSpr2 	= precache_model(SPRITE_EXPLO[1]);
	exploSpr3 	= precache_model(SPRITE_EXPLO[2]);
	exploSpr4 	= precache_model(SPRITE_EXPLO[3]);
	exploSpr5 	= precache_model(SPRITE_EXPLO[4]);
	gibSpr 		= precache_model(SPRITE_GIB[0]);
	gibSpr2 	= precache_model(SPRITE_GIB[1]);
	gibSpr3 	= precache_model(SPRITE_GIB[2]);
}
...
public native_fn_get_user_chillspeed(plugin, params) {
	if(params != 1)
		return PLUGIN_CONTINUE;

	new id = get_param(1);

	return get_default_maxspeed(id)*(get_pcvar_float(pcv_chill_speed)/100.0);
}

public native_fn_is_user_chilled(plugin, params) {
	if(params != 1)
		return PLUGIN_CONTINUE;

	new id = get_param(1);

	return bool:isChilled[id];
}

public native_fn_get_chill_duration(plugin, params) {
	new Float:duration = get_pcvar_float(pcv_chill_duration)

	return duration;
}

public native_fn_is_user_frozen(plugin, params) {
	if(params != 1)
		return PLUGIN_CONTINUE;

	new id = get_param(1);

	return bool:isFrozen[id];
}
...
create_nova(id)
{
	...
	engfunc(EngFunc_SetSize,nova, Float:{ -3.0, -3.0, -3.0 }, Float:{ 3.0, 3.0, 3.0 });
	...
	novaOrigin[2] -= pev(id, pev_flags) & FL_DUCKING ? 15.0 : 35.0;
...
create_blast(team,Float:origin[3])
{
	...
	new randomNum = random_num(0, 4);
	new gibNum = random_num(0, 2);
	...
	write_short(randomNum == 0 ? exploSpr : randomNum == 1 ? exploSpr2 : randomNum == 2 ? exploSpr3 : randomNum == 3 ? exploSpr4 : exploSpr5); // sprite
	...
	write_short(gibNum == 0 ? gibSpr : gibNum == 1 ? gibSpr2 : gibSpr3) // Sprite Index 
	write_byte(80) // Count 
	write_byte(20) // Life 
	write_byte(2) // Scale 
...
set_beamfollow(ent)
{
	...
	write_short(trailNum == 0 ? trailSpr : trailSpr2) // sprite
	write_byte(10) // life
	write_byte(10) // width
	write_byte(0) // r
	write_byte(100) // g
