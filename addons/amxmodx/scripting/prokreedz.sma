...
public plugin_init()
{
	...
	kz_hud_color = register_cvar("kz_hud_color", "95 251 23")
	...
	kz_nvg_colors = register_cvar("kz_nvg_colors","95 251 23")
	...
	kz_register_saycmd("duel","kz_duel", 0)
		...
		for(new i=0;i<alivePlayers;i++)
		{
			if( timer_started[Alive[i]])
			{
				...
				if( ShowTime[Alive[i]] == 1 )
				{
					...
					set_hudmessage(95, 251, 23, -1.0, 0.0, 0, 0.5, 2.0, 0.08, 2.0, true)
					show_hudmessage(Alive[i], "%02d:%02d  [ %d cp | %d gc %s]",imin, isec,checknumbers[Alive[i]], gochecknumbers[Alive[i]], IsPaused[Alive[i]] ? "| *Paused*" : "")
				}
				else
			...
			if(specmode == 2 || specmode == 4)
			{
				...
				if(target != Dead[i])
					if(is_user_alive(target) && timer_started[target])
					{
						...
						set_hudmessage(95, 251, 23, -1.0, 0.0, 0, 0.5, 2.0, 0.08, 2.0, true)
						show_hudmessage(Dead[i], "%02d:%02d  [ %d cp | %d gc %s]",imin, isec, checknumbers[target], gochecknumbers[target], IsPaused[target] ? "| *Paused*" : "")
					}
			}
		}
	}
...
public FwdHamPlayerSpawn( id )
{
	...
	if(firstspawn[id])
	{
		ColorChat(id, GREEN,  "^x04TryHard - ^x01KZ ^x03(Average|Hard|Extreme)", prefix)
...
public client_disconnect(id)
{
	...
	IsInDuelWith[id] = 0;
	IsInDuelWith[playerId] = 0;

	DuelTimer[id] = 0.0;
	DuelTimer[playerId] = 0.0;
...
public kz_menu(id)
{
	...
	formatex(msgcheck, 63, "CP - \y#%i", checknumbers[id])
	formatex(msggocheck, 63, "GC - \y#%i",  gochecknumbers[id])
	formatex(msgpause, 63, "Pause - %s^n", IsPaused[id] ? "\yON" : "\rOFF" )

	menu_additem( menu, msgcheck, "1" )
	menu_additem( menu, msggocheck, "2" )
	menu_additem( menu, "Top15^n", "3")
	menu_additem( menu, "Start", "4")
	menu_additem( menu, "Timer", "5" )
	menu_additem( menu, msgpause, "6" )
	menu_additem( menu, "Invis", "7" )
	menu_additem( menu, "Spec", "8" )
	menu_additem( menu, "Reset", "9")
...
public kz_duel( id )
 {
	 if(IsInDuel[id]) {
		ColorChat(id, GREEN,  "%s^x01 %L", prefix, id, "KZ_IN_DUEL")

		return PLUGIN_HANDLED;
	 }

	 if(!DefaultStart) {
		ColorChat(id, GREEN,  "%s^x01 %L", prefix, id, "KZ_NO_START")
 ...
 public menu_handler( id, menu, item )
 {
    ...
    new playerId = getSelectedPlayerInMenu(menu, item);

    if (!IsInDuel[playerId])
    {
		new iName[32];
		get_user_name(playerId, iName, charsmax(iName));

        ColorChat(id, GREEN,  "%s^x01 %L", prefix, id, iName, LANG_PLAYER, "KZ_DUEL_REQUEST")

		displayDuelRequestMenu(playerId, id);
    } else {
		ColorChat(id, GREEN,  "%s^x01 %L", prefix, id, LANG_PLAYER, "KZ_IS_IN_DUEL")
...
{
	new playerId = find_player("k", getSelectedInteger(hMenu, item));
	return playerId && is_user_connected(playerId) ? playerId : 0;
}
...
displayDuelRequestMenu(const id, const duelWithId) {
	static szMenuTitle[32], szUserId[32];

	get_user_name(duelWithId, szUserId, charsmax(szUserId));

	formatex(szMenuTitle, charsmax(szMenuTitle), "\rDuel %s?", szUserId);

	formatex(szUserId, charsmax(szUserId), "%d", get_user_userid(duelWithId));

	new hMenu = menu_create(szMenuTitle, "duelRequestMenuHandler");
...
public duelRequestMenuHandler(const id, const hMenu, const item) {
	...
	if(IsInDuel[selectedPlayerId] || IsInDuel[id]) {
		return PLUGIN_HANDLED;
	} else if(item == 1) {
		static szName[32];
		get_user_name(id, szName, charsmax(szName));

		kz_chat(id, "%L", id, szName, LANG_PLAYER, "KZ_DUEL_REJECTED")
	   	return PLUGIN_HANDLED;
	}
	...
	IsInDuel[id] = true;
	IsInDuel[selectedPlayerId] = true;

	set_pev(id, pev_velocity, Float:{0.0, 0.0, 0.0})
	set_pev(id, pev_origin, DefaultStartPos)

	set_pev(selectedPlayerId, pev_velocity, Float:{0.0, 0.0, 0.0})
	set_pev(selectedPlayerId, pev_origin, DefaultStartPos)
	
	set_pev( id, pev_flags, pev( id, pev_flags ) | FL_FROZEN );
	set_pev( selectedPlayerId, pev_flags, pev( id, pev_flags ) | FL_FROZEN );

	set_task(3.0, "task_unfreeze_player", id)
	set_task(3.0, "task_unfreeze_player", selectedPlayerId)

	kz_chat(id, "%L", id, "KZ_START")
	kz_chat(selectedPlayerId, "%L", selectedPlayerId, "KZ_START")

	ColorChat(id, GREEN,  "%s^x01 %L", prefix, id, szName1, szName2, LANG_PLAYER, "KZ_DUEL_STARTING")
	ColorChat(selectedPlayerId, GREEN,  "%s^x01 %L", prefix, selectedPlayerId, szName1, szName2, LANG_PLAYER, "KZ_DUEL_STARTING")
	

	IsInDuelWith[id] = selectedPlayerId;
	IsInDuelWith[selectedPlayerId] = id;

	return PLUGIN_HANDLED;
}

public task_unfreeze_player(id) {
	set_pev( id, pev_flags, pev( id, pev_flags ) & ~ FL_FROZEN ) ;
}
 /*! KZ DUEL REQUEST */
...
{
	if(IsInDuel[id] && timer_started[id]) {
		kz_chat(id, "%L", id, "KZ_DUEL_CANNOTRESET")
	} else {
...
public finish_climb(id)
{
	...
	//crxranks_give_user_xp(id, _, "finish_map")
	...
	if(IsInDuel[id]) {
		...
		// If enemy player completed the map
		if(!timer_started[playerId] && !timer_started[id]) {
			new playerName[32], name[32];
			new imin,isec,ims, imin2, isec2, ims2;

			get_user_name(id, name, charsmax(name));
			get_user_name(playerId, playerName, charsmax(name));

			imin = floatround(DuelTimer[id] / 60.0, floatround_floor)
			isec = floatround(DuelTimer[id] - imin * 60.0,floatround_floor)
			ims = floatround( ( DuelTimer[id] - ( imin * 60.0 + isec ) ) * 100.0, floatround_floor )
			
			imin2 = floatround(DuelTimer[playerId] / 60.0, floatround_floor)
			isec2 = floatround(DuelTimer[playerId] - imin * 60.0,floatround_floor)
			ims2 = floatround( ( DuelTimer[playerId] - ( imin * 60.0 + isec ) ) * 100.0, floatround_floor )
			
			new minDiff = (imin > imin2 ? imin - imin2 : imin2 - imin);
			new secDiff = (isec > isec2 ? isec - isec2 : isec2 - isec);
			new msDiff = (ims > ims2 ? ims - ims2 : ims2 - ims);

			ColorChat(0, GREEN,  "%s^x01^x03 %s^x01 %L^x04 %02i:%02i.%02i ^x01--- ^x03 %s^x01 %L^x04 %02i:%02i.%02i ^x01! (^x03 Difference:^x04 %02i:%02i.%02i ^x01)", prefix, name, LANG_PLAYER, "KZ_FINISH_MSG", imin, isec, ims, playerName, LANG_PLAYER, "KZ_FINISH_MSG", imin2, isec2, ims2, minDiff, secDiff, msDiff)
...
public ProTop_show(id)
{
	...
	len += formatex(buffer[len], 2047-len, "<tr><Center><b><BR>Pro Top")
...
public NoobTop_show(id)
{
	...
	len += formatex(buffer[len], 2047-len, "</table></body>")
	len += formatex(buffer[len], 2047-len, "<tr><Center><b><BR>Noob Top")
